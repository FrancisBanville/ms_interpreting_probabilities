...
> generative Bayesian and machine learning models) to reconstruct networks at
> fine spatial and temporal scales with limited interaction data. For instance,
...
> interact at a specific location and time $l$ may be determined by the product
...
> using their individual (marginal) occurrence probabilities $P(X_{i,l})$ and
> $P(X_{j,l})$. Given that taxa occurrences are not independent of each other,
...
> 
> $$P(X_{i,l}, X_{j,l}) = P(X_{i,l} | X_{j,l}) P(X_{j,l})  = P(X_{i,l})
> P(X_{j,l})
...
> empirical webs, $\gamma > 1$ holds true for the majority of species pairs
...
> $\lambda_l$. This parameter represents the local expected frequency of
...
> units of $\lambda_l$ and $t_0$ are complementary. For instance, if the
> duration $t_0$ is measured in months, $\lambda_l$ denote the expected number
...
> estimate the value of the $\lambda_l$ and $\gamma$ parameters and generate
> novel interaction data:
> 
> $$L_{i \rightarrow j} \sim \text{Bernoulli}(P(L_{i \rightarrow j}))$$
> {#eq:model} 
> 
> $$P(L_{i \rightarrow j}) = P(X_{i,l}) P(X_{j,l}) \gamma (1-e^{-\lambda_l
> t_0})$$ {#eq:modeleq}
> 
...
> that, irrespective of the interaction rate $\lambda_l$, the probability of
> interaction converges toward an asymptote determined by the probability of
> co-occurrence $P(X_{i,j,l})$ (@Eq:modelcoprob). This model can be customized
