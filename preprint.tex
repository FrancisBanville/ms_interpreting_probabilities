...
important methodological and conceptual decisions must be made when
...
context in which interaction probabilities were estimated. This gives us
the following equation for the probability of realized interaction
between species (or taxa) \(i\) and \(j\) in a local network \(N\):
...
\item
  Sampling local networks from metawebs vs from probabilistic local
  networks
\end{itemize}
...
\leavevmode\hypertarget{ref-Pringle2020ResFoo}{}%
