#### Script adapted from
#### https://esajournals.onlinelibrary.wiley.com/doi/10.1002/ecy.1832 (ecy1832-sup-0002-AppendixS1.zip)
#### August 8th 2023
####################################
...
get_formatData <- function(filename) {
    ####################################### IMPORTING DATA
    data_salix <- read.csv(filename, header = FALSE, stringsAsFactors = FALSE,sep = ";")
    nr <- nrow(data_salix)
    nc <- ncol(data_salix)
